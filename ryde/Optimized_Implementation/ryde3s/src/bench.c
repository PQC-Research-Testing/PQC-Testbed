...
int main(void) {
    ...
    unsigned long long ryde_3s_keypair_mean = 0, ryde_3s_crypto_sign_mean = 0, ryde_3s_crypto_sign_open_mean = 0;
...
//    (void)syscall(SYS_getrandom, seed, 48, 0);
    ...
    for(size_t i = 0 ; i < NB_TEST ; i++) {
        crypto_sign_keypair(ryde_3s_pk, ryde_3s_sk);
    }
    ...
    for(size_t i = 0 ; i < NB_SAMPLES ; i++) {
        ...
        for(size_t j = 0 ; j < NB_TEST ; j++) {
            ...
            t1 = cpucyclesStart();
            crypto_sign(ryde_3s_sm, &ryde_3s_smlen, ryde_3s_m, ryde_3s_mlen, ryde_3s_sk);
            t2 = cpucyclesStop();

            timer += t2 - t1;
        }

        ryde_3s_crypto_sign_mean += timer / NB_TEST;
    }
    printf("Benchmark (crypto_sign)\n");
    ...
    for(size_t i = 0 ; i < NB_SAMPLES ; i++) {
        ...
        crypto_sign_keypair(ryde_3s_pk, ryde_3s_sk);
        crypto_sign(ryde_3s_sm, &ryde_3s_smlen, ryde_3s_m, ryde_3s_mlen, ryde_3s_sk);
        if (crypto_sign_open(ryde_3s_m, &ryde_3s_mlen, ryde_3s_sm, ryde_3s_smlen, ryde_3s_pk) == -1) { ryde_3s_failures++; }
        timer = 0;
        ...
        for(size_t j = 0 ; j < NB_TEST ; j++) {
            ...
            t1 = cpucyclesStart();
            crypto_sign_open(ryde_3s_m, &ryde_3s_mlen, ryde_3s_sm, ryde_3s_smlen, ryde_3s_pk);
            t2 = cpucyclesStop();

            timer += t2 - t1;
        }

        ryde_3s_crypto_sign_open_mean += timer / NB_TEST;
    }
    printf("Benchmark (crypto_sign_open)\n");
    ...
    printf("\n  crypto_sign_keypair: %lld CPU cycles", ryde_3s_keypair_mean / NB_SAMPLES);
    printf("\n  crypto_sign:         %lld CPU cycles", ryde_3s_crypto_sign_mean / NB_SAMPLES);
    printf("\n  crypto_sign_open:    %lld CPU cycles", ryde_3s_crypto_sign_open_mean / NB_SAMPLES);
    printf("\n\n");
