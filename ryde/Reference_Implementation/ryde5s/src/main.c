...
//    (void)syscall(SYS_getrandom, seed, 48, 0);
    ...
    t1 = cpucyclesStart();
    if (crypto_sign_keypair(ryde_5s_pk, ryde_5s_sk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t2 = cpucyclesStop();
    ...
    t3 = cpucyclesStart();
    if (crypto_sign(ryde_5s_sm, &ryde_5s_smlen, ryde_5s_m, ryde_5s_mlen, ryde_5s_sk) != 0) {
        printf("\nnFailed\n\n");
        return -1;
    };
    t4 = cpucyclesStart();
    ...
    t5 = cpucyclesStart();
    if (crypto_sign_open(ryde_5s_m, &ryde_5s_mlen, ryde_5s_sm, ryde_5s_smlen, ryde_5s_pk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t6 = cpucyclesStart();
