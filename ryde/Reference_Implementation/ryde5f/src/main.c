...
//    (void)syscall(SYS_getrandom, seed, 48, 0);
    ...
    t1 = cpucyclesStart();
    if (crypto_sign_keypair(ryde_5f_pk, ryde_5f_sk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t2 = cpucyclesStop();
    ...
    t3 = cpucyclesStart();
    if (crypto_sign(ryde_5f_sm, &ryde_5f_smlen, ryde_5f_m, ryde_5f_mlen, ryde_5f_sk) != 0) {
        printf("\nnFailed\n\n");
        return -1;
    };
    t4 = cpucyclesStart();
    ...
    t5 = cpucyclesStart();
    if (crypto_sign_open(ryde_5f_m, &ryde_5f_mlen, ryde_5f_sm, ryde_5f_smlen, ryde_5f_pk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t6 = cpucyclesStart();
