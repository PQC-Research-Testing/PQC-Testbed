...
//    (void)syscall(SYS_getrandom, seed, 48, 0);
    ...
    t1 = cpucyclesStart();
    if (crypto_sign_keypair(mirath1as_pk, mirath1as_sk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t2 = cpucyclesStop();
    ...
    t3 = cpucyclesStart();
    if (crypto_sign(mirath1as_sm, &mirath1as_smlen, mirath1as_m, mirath1as_mlen, mirath1as_sk) != 0) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t4 = cpucyclesStart();
    ...
    t5 = cpucyclesStart();
    if (crypto_sign_open(mirath1as_m, &mirath1as_mlen, mirath1as_sm, mirath1as_smlen, mirath1as_pk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t6 = cpucyclesStart();
