...
//    (void)syscall(SYS_getrandom, seed, 48, 0);
    ...
    t1 = cpucyclesStart();
    if (crypto_sign_keypair(mirath3as_pk, mirath3as_sk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t2 = cpucyclesStop();
    ...
    t3 = cpucyclesStart();
    if (crypto_sign(mirath3as_sm, &mirath3as_smlen, mirath3as_m, mirath3as_mlen, mirath3as_sk) != 0) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t4 = cpucyclesStart();
    ...
    t5 = cpucyclesStart();
    if (crypto_sign_open(mirath3as_m, &mirath3as_mlen, mirath3as_sm, mirath3as_smlen, mirath3as_pk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t6 = cpucyclesStart();
