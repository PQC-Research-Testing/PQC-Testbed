...
//    (void)syscall(SYS_getrandom, seed, 48, 0);
    ...
    t1 = cpucyclesStart();
    if (crypto_sign_keypair(mirath3af_pk, mirath3af_sk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t2 = cpucyclesStop();
    ...
    t3 = cpucyclesStart();
    if (crypto_sign(mirath3af_sm, &mirath3af_smlen, mirath3af_m, mirath3af_mlen, mirath3af_sk) != 0) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t4 = cpucyclesStart();
    ...
    t5 = cpucyclesStart();
    if (crypto_sign_open(mirath3af_m, &mirath3af_mlen, mirath3af_sm, mirath3af_smlen, mirath3af_pk) == -1) {
        printf("\nnFailed\n\n");
        return -1;
    }
    t6 = cpucyclesStart();
